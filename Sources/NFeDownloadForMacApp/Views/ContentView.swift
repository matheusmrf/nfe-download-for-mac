        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                HeroCard(
                    ...
                    subtitle: "Cole as chaves de acesso, escolha a pasta de saída e baixe os XMLs diretamente do serviço SAP."
                )
                ...
                card {
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        Text("Chaves de acesso")
                            .font(.headline)
                        TextEditor(text: $viewModel.downloadKeysText)
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                HeroCard(
                    ...
                    subtitle: "Selecione a pasta com XMLs e envie para o endpoint configurado de NF-e ou CT-e."
                )

                card {
                    VStack(alignment: .leading, spacing: 16) {
                        Picker("Tipo de serviço", selection: $viewModel.selectedServiceKind) {
                        ...
                        LabeledContent(viewModel.selectedServiceKind.endpointTitle, value: viewModel.settings.endpoint(for: viewModel.selectedServiceKind))
                            .font(.callout)
                            .lineLimit(2)
                ...
                card {
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        TextField("Usuário", text: Binding(
                            ...
                            set: { viewModel.settings.username = $0 }
                        ))
                        SecureField("Senha", text: Binding(
                            get: { viewModel.settings.password },
                            set: { viewModel.settings.password = $0 }
                        ))
                    }
                }
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        HStack {
                            TextField("Pasta de download", text: Binding(
                                ...
                                set: { viewModel.settings.downloadDirectory = $0 }
                            ))
                        ...
                        HStack {
                            TextField("Pasta de envio", text: Binding(
                                ...
                                set: { viewModel.settings.sendDirectory = $0 }
                            ))
